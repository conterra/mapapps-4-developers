...
test('expect 2D widget shows zoom and rotation', async ({ page }, testInfo) => {
...
test('expect 3D widget shows zoom, heading and tilt', async ({ page }, testInfo) => {
    const sampleApp = new SampleApp(page);
    await sampleApp.goTo();
    await sampleApp.openCameraWidget();
